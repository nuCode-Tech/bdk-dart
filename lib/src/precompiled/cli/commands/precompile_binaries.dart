...
import 'package:bdk_dart/src/precompiled/rust_toolchain.dart';
...
''');
  ...
  if (normalizedOs == 'android') {
    ...
    for (final target in buildableTargets) {
      ...
      final soPath = p.join(buildOut.path, abi, 'lib$cratePackage.so');
    ...
    for (final target in buildableTargets) {
      ...
      for (final ent in artDir.listSync(followLinks: false)) {
        ...
        final ok =
            name.startsWith('lib$cratePackage.') ||
            name.startsWith('$cratePackage.') ||
            name == 'lib$cratePackage.a';
